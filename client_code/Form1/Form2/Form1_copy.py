...
class Form1_copy(Form1_copyTemplate):
    ...
    for pin in pins:
      self.markerList.append(
        GoogleMap.Marker(
          ...
          position=GoogleMap.LatLng(pin["Lat"], pin["Lon"]),
        )
      )
  ...
  def marker_click(self, sender, **properties):
    ...
    self.label_6.text = self.pictures[self.count_click % len(self.pictures)][
      "Description"
    ]
    self.load_wall()
  ...
  def signBtn_click(self, **event_args):
    if len(self.text_box_1.text.strip()) > 0:
      ...
      tables.app_tables.wall.add_row(
        Signer=self.text_box_1.text.strip(), When=now, Location=self.location
      )
  ...
  def drop_down_1_change(self, **event_args):
    self.image_1.source = self.pictures[
      self.drop_down_1.selected_value % len(self.pictures)
    ]["Image"]
